...
def trip(db: DBConnection, trip_id: str) -> str:
    with db.raw_execute(
        "SELECT trips.short_name, routes.short_name "
        ...
        trip_name, route_name = cast(tuple[str, str], row)
    ...
    if first_stop:
        return f"trip {trip_id!r} ({route_name} {trip_name}, {first_stop} -> {last_stop})"
    return f"trip {trip_id!r} ({route_name} {trip_name})"
